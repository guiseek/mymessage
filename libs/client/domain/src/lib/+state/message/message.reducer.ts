...
const messageReducer = createReducer(
  initialState,
  on(MessageActions.loadMessage,
    state => ({ ...state, loaded: false, error: null })
  ),
  on(MessageActions.loadMessageSuccess,
    (state, { message }) => messageAdapter.upsertMany(message, { ...state, loaded: true })
  ),
  on(MessageActions.loadMessageFailure,
